...
test("frozen production rendering baseline", async ({ page, browser }) => {
  ...
  const assetManifest = readFileSync("production-assets-b6.json");
  const report = {
    ...
    benchmark_script_sha256: createHash("sha256").update(readFileSync("tests/b6-performance.spec.ts")).digest("hex"),
